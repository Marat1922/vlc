...
class InfoActivity : AudioPlayerContainerActivity(), View.OnClickListener, PathAdapterListener,
    ...
    override val insetListener = { insets: Insets ->
        (binding.mlItemResolution.layoutParams as ConstraintLayout.LayoutParams).topMargin =
            insets.top + 16.dp
    }
...
//                    if (fabVisibility != -1) binding.fab.visibility = fabVisibility
        ...
        binding.directoryNotScannedButton.setOnClickListener {
            ...
            Snackbar.make(
                binding.root,
                getString(R.string.scanned_directory_added, parent.toUri().lastPathSegment),
        ...
        when {
            item.itemType == MediaLibraryItem.TYPE_MEDIA -> {
                ...
                if (isSchemeSupported(media.uri?.scheme)) {
                    ...
                    binding.ariane.layoutManager = LinearLayoutManager(
                        this@InfoActivity,
                    ...
                    if (binding.ariane.itemDecorationCount == 0) {
                        binding.ariane.addItemDecoration(
            ...
            item.itemType == MediaLibraryItem.TYPE_ARTIST -> {
                ...
                binding.sizeIcon.setImageDrawable(
                    ContextCompat.getDrawable(
                        this@InfoActivity,
                        R.drawable.ic_album
                    )
                ...
                binding.extraIcon.setImageDrawable(
                    ContextCompat.getDrawable(
                        this@InfoActivity,
                        R.drawable.ic_song
                    )
...
//    }
    ...
    class InfoModel : ViewModel() {
            ...
            item?.let { item ->
                cover.value = item.artworkMrl?.let {
                    withContext(Dispatchers.IO) {
                        AudioUtil.fetchCoverBitmap(
                            Uri.decode(it),
                            width
                        )
                    }
                } ?: (item as? MediaWrapper)?.let { media ->
                    if (item.type == MediaWrapper.TYPE_VIDEO) withContext(Dispatchers.IO) {
                        ThumbnailsProvider.getVideoThumbnail(
                            media,
                            width
                        )
                    } else null
        ...
        fun checkFile(mw: MediaWrapper) = viewModelScope.launch {
            val itemFile =
                withContext(Dispatchers.IO) { File(Uri.decode(mw.location.substring(5))) }
